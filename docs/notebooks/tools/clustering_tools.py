...
def plot_dendrogram(
    ...
    scipy.cluster.hierarchy.dendrogram(
        linkage_matrix, labels=labels, count_sort="descending"
    )
    plt.title(title)
    plt.ylabel("Distance")
    plt.show()
...
def generate_toy_datasets(
    ...
    moons = make_moons(
        n_samples=n_samples, noise=0.05, random_state=random_state_1
    )[0]
    blobs = make_blobs(
        n_samples=n_samples, centers=2, random_state=random_state_1
    )[0]
    rng = np.random.RandomState(random_state_1)
    no_structure = rng.rand(n_samples, 2)

    X = make_blobs(
        n_samples=n_samples, centers=2, random_state=random_state_2
    )[0]
